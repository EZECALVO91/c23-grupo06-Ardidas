...
const usersController = {
  ...
  usersUpdate: (req, res) => {
    ...
    const { name, email, password, category, date, localidad, sobremi } =
      req.body;
  ...
  userProfileEdit: (req, res) => {
    ...
    //   const { id } = req.params;
      ...
      db.User.update({
        ...
        localidad,
        sobremi,
        image: req.file ? req.file.filename : req.session.usuarioLogin.image
        //Se llamaba asi la imagen??
      },{
          where: {id: req.session.usuarioLogin.id}
      })
      .then(() =>{
          return res.redirect("/")
      })
